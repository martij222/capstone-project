...
master <- as_tibble(read.csv("basketball_master.csv")) # Master list containing names and playerID's (among other biographical data)
...
The *awards* data set seemed to merge correctly, but it looks like the number of rows changed after the *allstar* merge. To investigate this error, we can use the `anti_join()` function to compare the original *allstar* data set to the entries that were actually transferred over. `anti_join()` only returns rows from the primary data frame that are *not* in the secondary, while keeping the columns from the primary.
...
With this, we've taken care of the many discrepancies that arose from the attempted joining of the *allstar* and *player* data. At this point we can be confident that we can safely merge all data sets into a common *players* data set.
...
``` {r}
write.csv(players.joined, "players_clean.csv")
```
